...
(function(PH) {
    ...
    //     ["Acts"]
    // ];
    var totalLengths = [107,187,78,107,187,78,107,187,78,150];
    var plan = [
    ...
    var bookAndChapter = function(index, group) {
        ...
        return { "book": group[bookIndex], "chapter": index, "id": group[bookIndex]+index };
    };
    PH.getReadingPlan = function(date) {
        date.setHours(0,0,0,0); // set to midnight
        index = Math.floor(date / 86400000) % PH.lcm;
        var modifiers = [0, 0, 0, 1, 1, 1, 2, 2, 2, 0];
        var indexes = totalLengths.map(function(length) { return index > length ? (index % length) + 1 : index; });
        indexes = _.zip(indexes, modifiers).map(function(i) { return i[0] + i[1]; })
